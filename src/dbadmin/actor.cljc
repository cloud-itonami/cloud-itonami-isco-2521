(ns dbadmin.actor
  "DatabaseAdministrationActor — the ISCO-08 2521 community dbadmin actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one dbadmin operation request
  ...
  The unconditional invariant: the DatabaseAdministrationAdvisor can never
  directly commit a record the DatabaseAdministrationGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [dbadmin.advisor :as advisor]
            [dbadmin.governor :as governor]
            [dbadmin.store :as store]))

(defn build-graph
  "Build a compiled DatabaseAdministrationActor graph. `store` implements
  `dbadmin.store/Store`. `advisor` implements
  `dbadmin.advisor/Advisor` (defaults to `mock-advisor`).
